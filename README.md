...
Esta aplicação é um rastreador de múltiplas paragens de autocarro (do inglês **Multiple Bus Stop Tracker** (MBST)) para a rede da Carris Metropolitana. Recorrendo à API da Carris Metropolitana, esta aplicação retorna os dados recebidos desta que inclui alguns dados importante para o utilizador (hora de partida, linha, destino, etc.), podendo retornar apenas uma ou múltiplas paragens ao mesmo tempo, em poucos cliques.
...
- **1. Os IDs são valores mais estáveis do que os nomes**
  ...
  Um utilizador que está mais atento aos detalhes das paragens à sua volta (seja no terreno, seja no [website](https://carrismetropolitana.pt)) conseguirá perceber que algumas paragens mudaram os seus nomes desde o início da operação da Carris Metropolitana. Caso a aplicação utilizasse os nomes para buscar os valores das paragens, algumas destas poderiam não funcionar corretamente, enquanto os IDs são valores mais estáveis para retornar os dados corretamente.
...
- **Carris Metropolitana**
  No caso apresentado, em que as linhas 1704 e 3715 têm nomes diferentes para o mesmo destino (Marquês de Pombal), o resultado apresentado provém diretamente de todos os dados recebidos da API da Carris Metropolitana, sendo por tal algo relativo com esta.
...
1. Entrando no website, irá deparar-se com este menu:
   ...
   **Figura 1:** Menu do website
   ...
   No menu irá deparar-se com uma caixa de texto, onde o utilizador irá introduzir os IDs das paragens que desejar. Como já referido, os IDs das paragens serão os valores bases para utilizar no website. Caso o utilizador esteja em dúvida o que seria o ID de uma paragem:
   ...
   Caso o utilizador já tenho feita uma pesquisa anteriormente, os últimos dados serão guardados automaticamente numa cookie que guarda os IDs das paragens, garantindo assim que o utilizador não tenha de voltar a escrever os IDs sempre que tenha de reentrar no website a não ser que faça uma nova pesquisa. De notar que, quantas mais paragens serem pedidas, mais tempo o pedido poderá retornar resultados.
...
2. Após pesquisar as paragens, irão aparecer as paragens mais abaixo na página com este formato:
   ...
   **Figura 4.2:** Exemplo de uma tabela individual com os detalhes adicionais
   ...
   Note que as tabelas mostrarão apenas os próximos 10 horários que estão dentro dos próximos 60 minutos. Se nenhum resultado for encontrado condizer com essas condições, nenhum horário será retornado, como por exemplo:
   - Se uma pesquisa for feita às 3h da manhã, a maioria das paragens não terá resultados retornados porque a maioria dos autocarros não estaria a circular a essa hora nem nos próximos 60 minutos (fora algumas linhas na madrugada).
   - Se uma pesquisa for feita numa paragem com várias linhas na hora de ponta, é provável que retorne imensos horários nos próximos 60 minutos, fazendo a app ter de filtrar apenas os próximos 10 horários, evitando poluir visualmente o site dando maior destaque aos mais próximos.
...
This app is a **Multiple Bus Stop Tracker** (MBST) for the Carris Metropolitana network. Using the Carris Metropolitana API, this app retrieves data from the service, including key information for the user (departure time, route, destination, etc.), and can display details for a single stop or multiple stops simultaneously, in just a few clicks.
...
- **Carris Metropolitana**
  In the case described, where routes 1704 and 3715 have different names for the same destination (Marquês de Pombal), the result shown is derived directly from all the data received from the Carris Metropolitana API and is therefore dependent on it.
...
1. When you enter the website, you will see this menu:
   ...
   **Figure 1:** Website menu
   ...
   In the menu, you will see a text box where you can enter the IDs of the stops you want. As mentioned above, the stop IDs will be the base values to use on the website. If you are unsure what a stop ID is:
   ...
   If the user has already performed a search previously, the last data will be automatically stored in a cookie that saves the stop IDs, thus ensuring that the user does not have to re-enter the IDs each time they return to the website unless you do a new search. Please note that the more stops you request, the longer it will take for the request to return results.
...
2. After searching for stops, the stops will appear further down the page in this format:
   ...
   **Figure 4.2:** Example of an individual table with the additional details
   ...
   Please note that the tables will only show the next 10 departures within the next 60 minutes. If no results are found that match these conditions, no timetables will be returned, for example:
   - If a search is carried out at 3am, most stops will return no results because most buses would not be running at that time or within the next 60 minutes (apart from a few dawn routes).
   - If a search is carried out at a stop with several routes during rush hour, it is likely to return a large number of timetables within the next 60 minutes, requiring the app to filter down to just the next 10 timetables, thereby avoiding visual clutter on the site and highlighting the nearest ones.
   ...
   In the example shown above, route 3717 is running on time according to the timetable, route 4725 is ahead of schedule, route 3721 is slightly ahead of schedule, route 3705 is slightly behind schedule, and routes 3703 and 3710 are behind schedule. The remaining routes (shown in grey) in this case have not yet started service (or real-time data is disabled or there is an error). It should also be noted that, further down, as the 3717 bus is approaching the stop, the background of its entry is flashing as expected.
